...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (targetDesc) {
        let descIndex = 0;
        let charIndex = targetDesc.textContent.length; // Démarre à la fin du texte déjà écrit dans le HTML
        let isDeleting = true; // Commence par effacer la première phrase d'origine
        
        const typingSpeed = 30;   // Écriture rapide
        const deletingSpeed = 15;  // Effacement très rapide
        const readDelay = 4500;    // Temps de lecture de 4,5 secondes
        ...
        function handleDescTypewriter() {
            ...
            // Si le texte est entièrement écrit, on fait une pause de lecture
            if (!isDeleting && charIndex === currentFullDesc.length) {
                currentSpeed = readDelay;
                isDeleting = true;
            } 
            // Si le texte est entièrement effacé, on passe à la phrase suivante
            else if (isDeleting && charIndex === 0) {
                isDeleting = false;
                descIndex = (descIndex + 1) % descriptions.length;
                currentSpeed = 400; // Petite pause avant de réécrire
            }

            setTimeout(handleDescTypewriter, currentSpeed);
        }

        // Lancement de la machine à écrire après 1,5 seconde
        setTimeout(handleDescTypewriter, 1500);
    }
